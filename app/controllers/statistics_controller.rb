  ...
  def cal_response(num_responses, sd_id, survey)     #define a function to calculate number of responses
      ...
      for i in survey.min_question_score..survey.max_question_score
        if(total_question_response>0)
          num_responses[q.id][i]=(SurveyResponse.find(:all,:conditions=>["survey_deployment_id=? and question_id=? and score=?",sd_id,q.id,i]).length.to_f/total_question_response)
        else
          num_responses[q.id][i]=0.0
         end
      end
